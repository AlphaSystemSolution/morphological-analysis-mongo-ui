...
public class DictionaryNotesSkin extends SkinBase<DictionaryNotesView> {
    ...
    static {
        try {
            write(get(DEFAULT_CSS_DIRECTORY.getAbsolutePath(), CSS_RESOURCE_PATH),
                    readAllLines(String.format("%s.%s", ASCII_DOCTOR_RESOURCE_PATH, CSS_RESOURCE_PATH)));
    ...
    private void initializeSkin() {
        ...
        previewTab.selectedProperty().addListener((o, ov, nv) -> {
            saveAction();
        });
    ...
    private ToolBar createToolBar() {
        ...
        Button keyboardButton = createButton(KEYBOARD_ALT, "Show Keyboard", event -> showKeyboard(event));
    ...
    private void populateEditor(DictionaryNotes dictionaryNotes) {
        ...
        try {
            ...
            text = text.replace("${ArticleName}", replacement);
            editor.setText(text);
            saveAction();
        } catch (IOException | URISyntaxException e) {
            e.printStackTrace();
